...
- 이슈 생성
  ...
  ```
  Github의 issue 탭에 들어가서 기능 정의서를 기반으로 issue를 생성합니다.
  commit message rules와 동일하게 이슈를 생성해주세요.
  이슈를 생성하면 자동으로 번호가 발급됩니다. 이 번호를 사용하여 feature 브랜치를 생성합니다.
  ```

- 브랜치를 생성하여 해당 브랜치에 commit, push 합니다.

- PR을 날릴 때 작성자와 리뷰어를 설정합니다.

- 이 때 **resolved #이슈번호**로 작성하면 merge될 때 해당 issue는 자동 삭제됩니다.

- 이렇게 PR이 생성되면 새로운 issue번호가 부여됩니다. 즉, **PR도 issue입니다.**
